...
fn vs_main(in : VertexInput) -> VertexOutput {
  var position = in.position + in.quad_pos / vec2f(render_params.right, render_params.up) * 10;
...
fn fs_main(in : VertexOutput) -> @location(0) vec4<f32> {
  return 2 * vec4f(97.0/255, 41.0/255, 81.0/255, 0.5);
}
...
fn simulate_step(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
  ...
  let theta = perlin_noise3(vec3f(particle.position, iter.val)) *  2 * 3.14159;

  particle.velocity = particle.velocity + sim_params.magnitude * vec2(cos(theta), sin(theta));
  particle.velocity.x = clamp(particle.velocity.x, -.05, .05);
  particle.velocity.y = clamp(particle.velocity.y, -.05, .05);
